...
@patch("scripts.download.validate_url", side_effect=lambda u: u)
@patch("scripts.download.subprocess.run")
def test_download_video_invokes_yt_dlp_with_target_path(mock_run, _mock_validate, tmp_path):
...
def test_copy_local_copies_into_library(tmp_path):
    """Default behavior is now copy (not symlink) — symlinks can leak the work
    dir to attacker-controlled storage when archived/uploaded."""
    src = tmp_path / "in.mp4"
